...
def slow_FT(dynspec, freqs):
    ...
    if os.path.isfile(filename):
        ...
        print("C-implentation fit_1d-response not installed, computing slowFT with numpy")
        ft = np.fft.fftfreq(ntime, 1)

        # Scaled array of t * f/fref
        tscale = t[:,np.newaxis] * fscale[np.newaxis,:]
        FTphase = -2j*np.pi*tscale[:,np.newaxis,:] * ft[np.newaxis,:,np.newaxis]
