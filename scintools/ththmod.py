...
def rev_map(thth, tau, fd, eta, edges):
    ...
    fd_edges = (np.linspace(0, fd.shape[0], fd.shape[0]+1)-.5) * \
        (fd[1]-fd[0]).value+fd[0].value
    tau_edges = (np.linspace(0, tau.shape[0], tau.shape[0]+1)-.5) * \
        (tau[1]-tau[0]).value+tau[0].value

    recov = np.histogram2d(np.ravel(fd_map),
                           np.ravel(tau_map),
                           bins=(fd_edges, tau_edges),
                           weights=np.ravel(thth/np.abs(2*eta *
                                                        fd_map.T)).real)[0] + \
...
def G_revmap(w, V, eta, edges, tau, fd):
    th_cents = (edges[1:]+edges[:-1])/2
    th_cents -= th_cents[np.abs(th_cents) == np.abs(th_cents).min()]
    screen = np.conjugate(V[:, np.abs(w) == np.abs(w).max()][:, 0] *
                          np.sqrt(w[np.abs(w) == np.abs(w).max()]))
    ...
    pnts = (fd_map > 0)*(tau_map > 0)*(fd_map < fd.shape[0]) * \
        (tau_map < tau.shape[0])
